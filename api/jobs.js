...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ─── MODE: batch ──────────────────────────────────────────────────────────
    // Verify a list of companies against live Adzuna postings in one request.
    // The results page fires this whenever results change. Returns a map keyed by
    // company NAME → { verified, applyUrl, realLocation, title } for the ones we
    // could match. Companies with no live posting are omitted; the client treats a
    // missing entry as "unverified", which is the correct/honest behavior.
    // (Was previously unhandled → fell through to "company" mode → 400 Missing company.)
    if (mode === "batch") {
      const companies = Array.isArray(body?.companies) ? body.companies.slice(0, 15) : [];
      const zip = body?.zip || "";
      const out = {};
      await Promise.allSettled(companies.map(async (c) => {
        ...
        const params = new URLSearchParams({
          ...
          results_per_page: "5",
          what: `${c?.role || "intern"} intern`,
          company: name,
          "content-type": "application/json"
        });
        if (zip) params.set("where", zip);
        const ctrl = new AbortController();
        const timer = setTimeout(() => ctrl.abort(), 6000);
        try {
          const r = await fetch(`https://api.adzuna.com/v1/api/jobs/us/search/1?${params}`, { signal: ctrl.signal });
          ...
          if (best && best.redirect_url) {
            out[name] = {
              verified: true,
              applyUrl: best.redirect_url,
              realLocation: best.location?.display_name || "",
              title: best.title || ""
            };
          }
        } catch {
      ...
      console.log(`[jobs] batch verified ${Object.keys(out).length}/${companies.length} companies near ${zip}`);
